    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500">Ranked by activity — quiz 40% · completion 30% · certs 20% · streak 10%</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border bg-white p-8 text-center text-sm text-zinc-500">No learners match these filters. Try clearing city or category.</div>
      ) : (
        <>
          <div className="overflow-hidden rounded-2xl border bg-white shadow-sm">
            ...
            {data.map((e) => {
              ...
              return (
                <div
                  key={e.learner.id}
                  className={`flex items-center gap-3 border-b px-3 py-3 last:border-0 sm:grid sm:grid-cols-[52px_1fr_148px_152px_92px] sm:gap-3 sm:px-4 ${e.rank <= 3 ? "bg-gradient-to-r from-white to-zinc-50/60" : ""}`}
                >
                  ...
                  <div className="ml-auto flex w-[132px] flex-col gap-1.5 sm:ml-0 sm:w-auto">
                    <div className="flex items-center justify-between text-xs">
                      <span className={`font-black tracking-tight ${params.ordering === "rank" ? "text-zinc-900" : "text-zinc-600"}`}>{e.rr} RR</span>
