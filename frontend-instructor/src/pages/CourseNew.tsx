...
export default function CourseNew() {
  ...
  const onSubmit = async (data: Form) => {
    try {
      ...
      setToast("Course created — now add curriculum");
        ...
        <div className="mt-4 rounded-[20px] bg-white p-6 shadow-sm sm:p-8">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="mt-6 grid gap-4">
              ...
              <div className="mt-1 flex gap-2">
                ...
                <label className="shrink-0 inline-flex items-center rounded-xl bg-[#0f172a] px-4 py-2.5 text-xs font-semibold text-white cursor-pointer hover:bg-black">
                  ...
                  <input type="file" accept="image/*" className="hidden" onChange={async (e) => {
                    ...
                    } catch (err) { alert(`Upload failed: ${err}`); } finally { setCoverUploading(false); e.target.value = ""; }
                  }} />
                </label>
              </div>
            ...
            <div className="flex gap-3">
              <button disabled={isSubmitting} className="rounded-full bg-[#0f172a] px-6 py-2.5 text-sm font-bold text-white disabled:opacity-60">{isSubmitting ? "Creating…" : "Create & continue"}</button>
